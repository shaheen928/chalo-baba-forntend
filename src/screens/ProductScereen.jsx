...
const ProductScreen = () => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      toast.error(err?.data?.message || err.error || "Failed to add review");
    }
  };
  ...
  if (error) {
    return (
      <div className="text-center py-12 px-4">
        ...
        <Link
          to="/"
          className="inline-block mt-4 text-slate-600 hover:underline text-sm font-medium"
        >
          Go Back to Home
        </Link>
      </div>
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-1">
        <div className="rounded-xl overflow-hidden shadow-sm border border-gray-100 bg-gray-50/50 w-full flex items-center justify-center">
          <img
            src={product?.image}
            alt={product?.name}
        ...
        <div className="flex flex-col gap-2.5 md:gap-4">
          ...
          <div className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm mt-1 sm:mt-4">
            ...
            {product?.countInStock > 0 && (
              <div className="flex justify-between items-center py-3 border-t border-gray-100 my-4">
                <span className="text-gray-700 font-semibold">Qty:</span>
                <div className="relative">
                  <select
                    className="block appearance-none w-24 bg-white border border-gray-300 hover:border-blue-500 px-4 py-2.5 pr-8 rounded-lg shadow-inner focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-600 text-gray-800 transition-colors"
                    value={qty}
      ...
      >
        ...
        <div className="my-4 px-1 sm:px-0">
          <label className="block text-slate-500 text-sm mb-1.5 font-medium">
            Comment
          </label>
          <textarea
            className="w-full bg-white text-slate-800 border border-gray-300 rounded-xl p-3 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-100 transition-all text-sm placeholder-gray-400"
            rows="3"
        ...
        <button
          ...
          className="btn btn-primary mt-2 bg-slate-800 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-700 transition-colors shadow-sm disabled:bg-slate-400"
        >
